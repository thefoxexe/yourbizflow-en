...
const NoteEditor = () => {
  ...
  const fetchNote = useCallback(async () => {
    ...
    if (error || !data) {
      toast({ variant: 'destructive', title: 'Erreur', description: 'Note introuvable.' });
      navigate('/notes');
    } else {
      setNote(data);
      setTitle(data.name);
  ...
  const handleSave = async () => {
    ...
    const { error } = await supabase
      .from('storage_items')
      .update({ name: title, content: content, updated_at: new Date().toISOString() })
      .eq('id', noteId);

    if (error) {
      toast({ variant: 'destructive', title: 'Erreur', description: "La note n'a pas pu être enregistrée." });
    } else {
