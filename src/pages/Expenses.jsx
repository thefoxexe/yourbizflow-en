...
const expenseCategories = [
  ...
  { value: "rent", label: "Rent and charges", icon: Home },
  { value: "software", label: "Software and subscriptions", icon: Tool },
  {
    value: "marketing",
    label: "Marketing and advertising",
    icon: ShoppingCart,
  },
  { value: "transport", label: "Transport and travel", icon: Plane },
  { value: "meals", label: "Meals and entertainment", icon: Utensils },
  { value: "taxes", label: "Taxes and charges", icon: PiggyBank },
];
...
const ExpenseForm = ({
  ...
  const displayDate = currentExpense.expense_date
    ? typeof currentExpense.expense_date === "string"
      ? parseISO(currentExpense.expense_date)
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            {currentExpense.id ? "Modify" : "Add"} an expense
          </DialogTitle>
        ...
        <div className="space-y-4 py-4">
            ...
            <Popover>
              <PopoverTrigger asChild>
                ...
                >
                  ...
                  {displayDate ? (
                    format(displayDate, "PPP", { locale: fr })
                  ) : (
          ...
          <div>
            <Label htmlFor="spent_by">Spent by (optional)</Label>
            <Input
              id="spent_by"
              value={currentExpense.spent_by || ""}
              onChange={(e) => handleChange("spent_by", e.target.value)}
...
const SubscriptionForm = ({
  ...
  const displayDate = currentSubscription.start_date
    ? typeof currentSubscription.start_date === "string"
      ? parseISO(currentSubscription.start_date)
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            {currentSubscription.id ? "Edit" : "Add"} a subscription
          </DialogTitle>
          <DialogDescription>
            Expenses will be generated automatically.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-4 py-4">
          <div>
            ...
            <Input
              ...
              placeholder="Ex: Spotify, ChatGPT..."
            />
          </div>
          <div>
            <Label htmlFor="sub-amount">Price ({currencySymbol})</Label>
            <Input
              id="sub-amount"
              type="number"
          ...
          <div>
            <Label htmlFor="sub_spent_by">Spent by (optional)</Label>
            <Input
              id="sub_spent_by"
              value={currentSubscription.spent_by || ""}
              onChange={(e) => handleChange("spent_by", e.target.value)}
              placeholder="Ex: Jean Dupont"
            />
          </div>
            ...
            {isLoading ? (
              ...
              subscriptions.map((sub) => (
                <TableRow key={sub.id}>
                  ...
                  <TableCell>
                    {format(parseISO(sub.start_date), "dd/MM/yyyy")}
                  </TableCell>
                  <TableCell className="text-right">
                    {currencySymbol}
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <CardDescription className="capitalize">
                      {sub.frequency === "monthly" ? "Monthly" : "Annual"} -
                      Début: {format(parseISO(sub.start_date), "dd MMM yyyy")}
                    </CardDescription>
                    {sub.spent_by && (
                      <CardDescription className="flex items-center mt-1">
                        <User className="w-3 h-3 mr-1.5" />
...
const Expenses = () => {
  ...
  const handleSaveExpense = async (expense) => {
    ...
    if (error) {
      ...
      toast({
        title: "Success",
        description: `Expense ${id ? "update" : "created"}.`,
      });
  ...
  const handleSaveSubscription = async (subscription) => {
    ...
    if (error) {
      toast({
        variant: "destructive",
        title: "Error",
        description: "The subscription could not be saved.",
      });
    } else {
      toast({
        title: "Success",
      ...
      if (newSubId) {
        ...
        if (functionError) {
          toast({
            ...
            description: "Unable to generate past expenses.",
          });
        } else {
          toast({
            title: "Processing completed",
            description: "Past expenses have been added.",
          });
          await fetchExpenses();
        }
      }
  ...
  const openExpenseForm = (expense = null) => {
    ...
    setEditingExpense(
      expense
        ...
        : { ...initialExpenseState, expense_date: date }
    );
  ...
  const openSubscriptionForm = (sub = null) => {
    ...
    setEditingSubscription(
      sub
        ? { ...sub, start_date: date }
        : { ...initialSubscriptionState, start_date: date }
    );
    ...
    <div className="space-y-8">
      ...
      >
        ...
        <p className="text-muted-foreground">
          Keep an eye on all your cash outflows.
        </p>
      </motion.div>
      ...
      <Tabs defaultValue="expenses" className="w-full">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            ...
            >
              <Repeat className="mr-2 h-4 w-4" /> New Subscription.
            </Button>
            <Button
        ...
        <TabsContent value="expenses">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                List of all your one-off transactions.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <ExpensesList
