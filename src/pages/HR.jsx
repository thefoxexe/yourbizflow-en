...
const PayslipDialog = ({
  ...
  const currencySymbol =
    companyProfile?.currency?.toUpperCase() === "USD"
      ? "$"
      : companyProfile?.currency?.toUpperCase() === "CHF"
      ? "CHF"
      : "€";
  ...
  const generatePayslip = () => {
    ...
    const period = `${
      monthNames[issueDate.getMonth()]
    } ${issueDate.getFullYear()}`;

    const addContent = () => {
      doc.setFontSize(22);
      doc.setFont(undefined, "bold");
      doc.text("Payslip", doc.internal.pageSize.getWidth() - 15, 30, {
        align: "right",
      });
      doc.setFontSize(12);
      doc.setFont(undefined, "normal");
      doc.text(period, doc.internal.pageSize.getWidth() - 15, 37, {
        align: "right",
      });

      doc.setFontSize(10);
      doc.setFont(undefined, "bold");
      doc.text(companyProfile?.company_name || "Your Company", 15, 50);
      doc.setFont(undefined, "normal");
      doc.text(
        companyProfile?.company_address || "Your company address",
        15,
        55
      );
      doc.text(companyProfile?.company_phone || "Phone", 15, 60);

      doc.setFont(undefined, "bold");
      doc.text(employee.name, doc.internal.pageSize.getWidth() - 15, 50, {
        align: "right",
      });
      doc.setFont(undefined, "normal");
      doc.text(employee.position, doc.internal.pageSize.getWidth() - 15, 55, {
        align: "right",
      });
      doc.text(
        `Date of hire: ${new Date(employee.hire_date).toLocaleDateString(
          "fr-FR"
        )}`,
      ...
      const { monthlyNet, deductions: taxDeductions } = calculateNetSalary(
        totalGross,
      ...
      const body = [
        ...
        ["Gross base salary", formatCurrency(grossSalary)],
        [
          "Overtime",
          `${formatCurrency(
            overtimePay
          )} (${overtimeHours}h to ${overtimeRate}%)`,
        ],
        ["Exceptional bonus", formatCurrency(parseFloat(bonus))],
        [
          { content: `Total Gross`, styles: { fontStyle: "bold" } },
          {
            content: formatCurrency(totalGross),
        ...
        [
          ...
          `-${formatCurrency(parseFloat(deductions))}`,
        ],
        [
          {
            content: "Net to pay",
          ...
          {
            content: formatCurrency(finalNet),
            styles: {
              fontStyle: "bold",
              fontSize: 12,
              fillColor: "#F9FAFB",
      ...
      doc.setFontSize(10);
      doc.text(
        `Payslip generated on ${issueDate.toLocaleDateString("fr-FR")}.`,
        15,
        doc.internal.pageSize.height - 10
      );
      doc.text(
        `Payment to be made on the IBAN: ${employee.iban || "Not specified"}`,
        doc.internal.pageSize.getWidth() - 15,
        doc.internal.pageSize.height - 10,
        { align: "right" }
      );
      ...
      doc.save(
        `Payroll_Sheet_${employee.name.replace(" ", "_")}_${period}.pdf`
      );
      toast({ title: "Success", description: "Payslip generated." });
    ...
    <Dialog>
      <DialogTrigger asChild>
        <DropdownMenuItem onSelect={(e) => e.preventDefault()}>
          <FileText className="w-4 h-4 mr-2" /> Generate pay slip
        </DropdownMenuItem>
      </DialogTrigger>
      <DialogContent className="sm:max-w-lg max-h-[90vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>Adjustments for {employee.name}</DialogTitle>
        </DialogHeader>
        <div className="flex-grow overflow-y-auto pr-4 -mr-4 space-y-6 py-4">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <TooltipProvider>
                    <ShadTooltip>
                      ...
                      <TooltipContent>
                        <p>
                          Overtime rate of increase. <br /> 25% is the legal
                          rate for the first 8 hours in France.
                        </p>
                      </TooltipContent>
                    </ShadTooltip>
                  </TooltipProvider>
          ...
          <div className="p-4 border rounded-lg space-y-4">
            <Label className="font-semibold">Payroll variables</Label>
  ...
  return (
    ...
    >
          ...
          <DropdownMenuContent align="end">
            ...
            {isGeneratingPayslip ? (
              ...
              <PayslipDialog
                ...
                onGenerationStateChange={setIsGeneratingPayslip}
              />
            )}
            <DropdownMenuItem
      ...
      <div className="grid grid-cols-2 gap-4 text-sm w-full mt-4">
        <div>
          ...
          <p className="font-semibold">
            {companyProfile?.currency?.toUpperCase() === "USD"
              ...
              ? "CHF"
              : "€"}
            {employee.gross_salary?.toLocaleString("fr-FR") || "N/A"}
          </p>
        </div>
        <div>
        ...
        <div>
          <p className="text-muted-foreground">Hiring</p>
          <p className="font-semibold">
...
const HR = () => {
  ...
  const handleSaveEmployee = async () => {
    if (!newEmployee.name || !newEmployee.gross_salary) {
      toast({
        ...
        description: "Name and gross salary are required.",
      });
      return;
    }
    const payroll_config = newEmployee.payroll_config
      .filter((item) => item.name && item.rate)
      .map((item) => ({ ...item, rate: parseFloat(item.rate) }));
    const employeeData = {
      ...newEmployee,
      user_id: user.id,
      gross_salary: Number(newEmployee.gross_salary),
      payroll_config,
    };
    const { error } = currentEmployee
      ? await supabase
          .from("employees")
          .update(employeeData)
    ...
    if (error) {
      toast({
        variant: "destructive",
        title: "Error",
        description: "The employee could not be registered.",
      });
    } else {
      toast({
        title: "Success",
        description: `Employee ${currentEmployee ? "updated" : "added"}.`,
      });
  ...
  const openDialog = (employee = null) => {
    ...
    const defaultConfig = [
      { name: "Social charges", rate: 22 },
      { name: "Income tax", rate: 10 },
    ];
    const payrollConfig =
      employee?.payroll_config && employee.payroll_config.length > 0
        ? employee.payroll_config
        : defaultConfig;
    setNewEmployee(
      employee
  ...
  const addPayrollConfigLine = () => {
    setNewEmployee({
      ...newEmployee,
      payroll_config: [...newEmployee.payroll_config, { name: "", rate: "" }],
    });
  };

  const removePayrollConfigLine = (index) => {
    const updatedConfig = newEmployee.payroll_config.filter(
      (_, i) => i !== index
    );
    setNewEmployee({ ...newEmployee, payroll_config: updatedConfig });
  };
  ...
  const payrollByDepartment = useMemo(() => {
    const data = employees.reduce((acc, emp) => {
      ...
      if (!acc[dept]) acc[dept] = { name: dept, "Salary Mass": 0 };
      acc[dept]["Salary Mass"] += emp.gross_salary || 0;
      return acc;
    }, {});
    ...
    <div className="space-y-8">
      ...
      >
        <div>
          ...
          <p className="text-muted-foreground">
            Manage your teams, salaries and payslips efficiently.
          </p>
        </div>
        <Button
          onClick={() => openDialog()}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <StatsCard
          ...
          value={`${currencySymbol}${stats.totalPayroll.toLocaleString(
            "fr-FR"
          )}`}
        ...
        />
        <StatsCard
          title="Gross Average Salary"
          value={`${currencySymbol}${stats.averageSalary.toLocaleString(
            "fr-FR",
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        >
          <h2 className="text-xl font-bold text-foreground">Your Employees</h2>
          {loading ? (
            <div className="space-y-4">
              {[...Array(2)].map((_, i) => (
                <div
                  key={i}
                  className="h-36 bg-card rounded-xl animate-pulse"
                ></div>
              ))}
            </div>
          ) : (
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-3xl max-h-[90vh] flex flex-col">
          ...
          <div className="flex-grow overflow-y-auto pr-4 -mr-4 grid gap-6 py-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label htmlFor="status">Contract type</Label>
                <select
                  id="status"
            ...
            <div className="space-y-4 p-4 border rounded-lg">
              <Label className="font-semibold">
                Payroll configuration (taxes, contributions, etc.)
              </Label>
              {newEmployee.payroll_config.map((item, index) => (
                <div key={index} className="flex items-center gap-2">
                  <Input
                    ...
                    value={item.name}
                    onChange={(e) =>
                      handlePayrollConfigChange(index, "name", e.target.value)
                    }
                  />{" "}
                  <Input
                    type="number"
                    ...
                    value={item.rate}
                    onChange={(e) =>
                      handlePayrollConfigChange(index, "rate", e.target.value)
                    }
                    className="w-28"
                  />
                  <Button
                    variant="ghost"
                    size="icon"
                    onClick={() => removePayrollConfigLine(index)}
                  >
