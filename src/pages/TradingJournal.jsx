...
import { PlusCircle, TrendingUp, TrendingDown, DollarSign, ArrowUp, ArrowDown, BrainCircuit, Loader2, Lock } from 'lucide-react';
      ...
      <DialogContent className="max-w-2xl bg-gray-900 border-gray-700">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-white">
            ...
            Vos Insights de Trading IA
          </DialogTitle>
          <DialogDescription className="text-gray-400">
            Analyse de vos performances pour vous aider à progresser.
          </DialogDescription>
        </DialogHeader>
        <div 
...
const TradingJournal = () => {
  ...
  const fetchTrades = async () => {
    ...
    if (error) {
      toast({ variant: 'destructive', title: 'Erreur', description: 'Impossible de charger les trades.' });
    } else {
      setTrades(data);
    }
    setLoading(false);
  ...
  const handleSubmit = async (e) => {
    ...
    if (limitReached) {
      toast({
        ...
        description: 'Passez au plan Pro pour ajouter des trades illimités.',
      });
    ...
    if (!newTrade.asset || !newTrade.pnl_amount) {
      toast({ variant: 'destructive', title: 'Champs requis', description: 'Veuillez remplir l\'actif et le P&L.' });
      return;
    }
    ...
    if (error) {
      ...
      toast({ title: 'Succès', description: 'Trade ajouté avec succès.' });
  ...
  const handleInsights = async () => {
    ...
    setInsights(`
      <div class="flex flex-col items-center justify-center p-8 text-center">
        ...
        <p class="text-lg font-semibold">Analyse de vos performances en cours...</p>
        <p class="text-sm text-muted-foreground">Notre IA épluche vos données pour dénicher des pépites.</p>
      </div>
    `);
    ...
    try {
        ...
        setInsights("<p>Une erreur est survenue lors de la génération de l'analyse. Veuillez réessayer.</p>");
        toast({
            variant: "destructive",
            title: "Erreur IA",
            description: "Impossible de contacter le service d'analyse.",
        });
    } finally {
  ...
  return (
    <div className="space-y-6">
      <Helmet>
        <title>Journal de Trading - YourBizFlow</title>
        <meta name="description" content="Suivez et analysez vos performances de trading avec précision." />
      </Helmet>

      <InsightsDialog isOpen={isInsightsOpen} onOpenChange={setIsInsightsOpen} insights={insights} />

      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
          <h1 className="text-3xl font-bold">Journal de Trading</h1>
          <p className="text-muted-foreground">Analysez vos performances pour optimiser votre stratégie.</p>
        </motion.div>
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="sm:max-w-[425px]">
            ...
            <form onSubmit={handleSubmit} className="grid gap-4 py-4">
              <Select onValueChange={(value) => handleSelectChange('trade_type', value)} defaultValue="long">
                <SelectTrigger><SelectValue placeholder="Type de trade" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="long">Long / Achat</SelectItem>
                  <SelectItem value="short">Short / Vente</SelectItem>
                </SelectContent>
              </Select>
              <Input name="asset" placeholder="Actif (ex: BTC/USD)" value={newTrade.asset} onChange={handleInputChange} />
              <Input name="pnl_amount" type="number" placeholder="P&L ($)" value={newTrade.pnl_amount} onChange={handleInputChange} />
              <Input name="pnl_percentage" type="number" placeholder="P&L (%)" value={newTrade.pnl_percentage} onChange={handleInputChange} />
              <DatePicker date={newTrade.open_date} setDate={(date) => handleDateChange('open_date', date)} label="Date d'ouverture" />
              <DatePicker date={newTrade.close_date} setDate={(date) => handleDateChange('close_date', date)} label="Date de fermeture (optionnel)" />
              <Textarea name="notes" placeholder="Notes sur la stratégie..." value={newTrade.notes} onChange={handleInputChange} />
        ...
        >
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-sm">
                Vous avez atteint les {tradeLimit} trades autorisés. Pour en ajouter plus et débloquer les analyses IA, passez au plan Pro.
              </p>
              <Button asChild variant="link" className="p-0 h-auto text-yellow-300 mt-1">
                <Link to="/subscription">Mettre à niveau maintenant</Link>
              </Button>
            </div>
          </div>
        </motion.div>
      ...
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
        <StatCard title="Aujourd'hui" value={stats.today.pnl.toFixed(2)} valuePrefix="$" icon={<DollarSign className="h-4 w-4 text-muted-foreground" />} />
        <StatCard title="30 Derniers Jours" value={stats.last30Days.pnl.toFixed(2)} valuePrefix="$" icon={<DollarSign className="h-4 w-4 text-muted-foreground" />} />
        <StatCard title="Depuis le Début" value={stats.allTime.pnl.toFixed(2)} valuePrefix="$" icon={<DollarSign className="h-4 w-4 text-muted-foreground" />} />
      </div>

      <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
        <Card>
          <CardHeader><CardTitle className="text-xs sm:text-sm font-medium">Win Rate (Total)</CardTitle></CardHeader>
          <CardContent><div className="text-xl sm:text-2xl font-bold text-green-500">{stats.allTime.winRate.toFixed(1)}%</div></CardContent>
        </Card>
        <Card>
          <CardHeader><CardTitle className="text-xs sm:text-sm font-medium">Trades Gagnants</CardTitle></CardHeader>
          <CardContent><div className="text-xl sm:text-2xl font-bold">{stats.detailed.winningTrades}</div></CardContent>
        </Card>
        <Card>
          <CardHeader><CardTitle className="text-xs sm:text-sm font-medium">Trades Perdants</CardTitle></CardHeader>
          <CardContent><div className="text-xl sm:text-2xl font-bold">{stats.detailed.losingTrades}</div></CardContent>
        </Card>
        <Card>
          <CardHeader><CardTitle className="text-xs sm:text-sm font-medium">Total Trades</CardTitle></CardHeader>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          ...
          <CardContent className="space-y-4 text-sm">
            <div className="flex justify-between"><span>Gain Moyen</span><span className="font-bold text-green-500">${stats.detailed.avgGain.toFixed(2)}</span></div>
            <div className="flex justify-between"><span>Perte Moyenne</span><span className="font-bold text-red-500">${stats.detailed.avgLoss.toFixed(2)}</span></div>
            <div className="flex justify-between"><span>Plus Gros Gain</span><span className="font-bold text-green-500">${stats.detailed.biggestGain.toFixed(2)}</span></div>
            <div className="flex justify-between"><span>Plus Grosse Perte</span><span className="font-bold text-red-500">${stats.detailed.biggestLoss.toFixed(2)}</span></div>
            <div className="flex justify-between"><span>Ratio Risk/Reward</span><span className="font-bold">{Math.abs(stats.detailed.avgGain / stats.detailed.avgLoss).toFixed(2) || 'N/A'}</span></div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={chartData}>
                ...
                <Line type="monotone" dataKey="pnl" name="P&L Cumulé" stroke="hsl(var(--primary))" strokeWidth={2} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        ...
        <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <CardTitle>Historique des Trades</CardTitle>
            <Button variant="outline" size="sm" onClick={handleInsights} disabled={isInsightsLoading || limitReached}>
              ...
              {limitReached ? 'Upgrade pour IA' : 'Insights IA'}
            </Button>
        </CardHeader>
        <CardContent>
            ...
            <Table>
              ...
              <TableBody>
                {loading ? (
                  ...
                  <TableRow><TableCell colSpan={4} className="text-center">Aucun trade enregistré.</TableCell></TableRow>
                )}
              </TableBody>
            </Table>
          </div>
