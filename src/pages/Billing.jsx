...
const Billing = () => {
  ...
  const handleCreateInvoice = async () => {
    ...
    if (!newInvoice.client_id || !newInvoice.due_date || totalAmount <= 0) {
      toast({
        ...
        title: "Champs requis",
        description:
          "Veuillez remplir tous les champs et ajouter au moins un article.",
      });
      return;
    }
    ...
    if (error) {
      toast({
        variant: "destructive",
        title: "Erreur",
        description: "La facture n'a pas pu être créée.",
      });
    } else {
  ...
  const handleSaveCompanyData = async () => {
    ...
    if (logoFile) {
      ...
      if (uploadError) {
        toast({
          ...
          description: "Le logo n'a pas pu être téléversé.",
        });
    ...
    if (error) {
      toast({
        variant: "destructive",
        title: "Erreur",
        description: "Les données n'ont pas pu être enregistrées.",
      });
    } else {
      toast({
        title: "Succès",
        description: "Données de l'entreprise mises à jour.",
      });
  ...
  const handleDeleteInvoice = async (invoiceId) => {
    ...
    if (error) {
      toast({
        variant: "destructive",
        title: "Erreur",
        description: "La facture n'a pas pu être supprimée.",
      });
    } else {
  ...
  const handleGenerateSuggestion = async (index) => {
    if (!profile?.business_description) {
      toast({
        ...
        description:
          "Veuillez d'abord décrire votre activité dans les données d'entreprise.",
      });
    ...
    if (error || data.error) {
      toast({
        ...
        description: "La suggestion n'a pas pu être générée.",
      });
    } else {
  ...
  const handleImportProjectHours = async (projectId) => {
    ...
    if (error || timeEntries.length === 0) {
      toast({
        title: "Information",
        description: "Aucune heure non facturée trouvée pour ce projet.",
      });
    ...
    toast({
      title: "Heures importées",
      description: `${totalHours.toFixed(
        2
      )} heures ont été ajoutées à une nouvelle facture.`,
    });
  };
  ...
  const handleDownloadPDF = (invoice) => {
    ...
    const addContent = () => {
      ...
      doc.text(
        profile?.email || user?.email || "contact@votre-entreprise.com",
        120,
        27
      );
      ...
      doc.text(
        `Date d'émission : ${new Date(invoice.issue_date).toLocaleDateString(
          "fr-FR"
        )}`,
        120,
        75
  ...
  return (
    <div className="space-y-8">
      <Helmet>
        ...
        <meta
          name="description"
          content="Gérez vos factures, suivez les paiements et créez des documents professionnels en quelques clics avec le module de facturation de YourBizFlow."
        />
      </Helmet>
      <motion.div
                ...
                >
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      <DropdownMenuSub>
                        <DropdownMenuSubTrigger>
                          <Edit className="w-4 h-4 mr-2" /> Statut
                        </DropdownMenuSubTrigger>
                        <DropdownMenuPortal>
                          <DropdownMenuSubContent>
                            <DropdownMenuItem
        ...
        <DialogContent className="sm:max-w-lg">
          ...
          {selectedInvoice && (
            <div className="py-4 space-y-4">
              ...
              <p>
                <strong>Montant:</strong> {currencySymbol}
      ...
      <Dialog open={isCompanyDialogOpen} onOpenChange={setIsCompanyDialogOpen}>
        <DialogContent className="sm:max-w-2xl flex flex-col max-h-[90vh]">
          <DialogHeader>
            ...
            <DialogDescription>
              Ces informations apparaîtront sur vos factures et aideront l'IA.
            </DialogDescription>
          </DialogHeader>
          <div className="flex-grow overflow-y-auto pr-6 -mr-6 grid gap-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                placeholder="Ex: Vente de vêtements de seconde main, services de conseil en marketing digital, création de sites web..."
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Sélectionnez un projet pour créer une facture à partir des heures
              non facturées.
            </DialogDescription>
          </DialogHeader>
          <div className="py-4 max-h-64 overflow-y-auto">
            {projects.length > 0 ? (
              ...
              <p className="text-muted-foreground text-center">
                Aucun projet avec un tarif horaire n'a été trouvé.
              </p>
            )}
          </div>
        </DialogContent>
