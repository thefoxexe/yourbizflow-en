...
const Analytics = () => {
  ...
  useEffect(() => {
    const fetchAnalyticsData = async () => {
      ...
      const [
        ...
        supabase
          .from("invoices")
          .select("amount, status, issue_date, client_id, due_date")
          .eq("user_id", user.id),
        supabase.from("quotes").select("id, status").eq("user_id", user.id),
        supabase
          .from("customer_subscriptions")
          .select(
            "*, subscription_product_id(price, interval), start_date, canceled_at, status"
          )
          .eq("user_id", user.id),
        supabase
          .from("revenues")
      ...
      const avgInvoice =
        paidInvoices.length > 0 ? totalInvoiceRevenue / paidInvoices.length : 0;

      const acceptedQuotes = quotesData.filter(
        (q) => q.status === "accepted"
      ).length;
      const conversionRate =
        quotesData.length > 0 ? (acceptedQuotes / quotesData.length) * 100 : 0;
      ...
      const clientLTV =
        clientsData.length > 0 ? totalRevenue / clientsData.length : 0;
      ...
      subscriptionsData.forEach((sub) => {
        ...
        Object.entries(monthlyData).forEach(([key, monthDataItem]) => {
          ...
          if (
            getYear(subStartDate) < getYear(monthDate) ||
            (getYear(subStartDate) === getYear(monthDate) &&
              getMonth(subStartDate) <= getMonth(monthDate))
          ) {
            if (sub.status === "canceled") {
              const cancelDate = new Date(sub.canceled_at);
              if (
                getYear(cancelDate) > getYear(monthDate) ||
                (getYear(cancelDate) === getYear(monthDate) &&
                  getMonth(cancelDate) > getMonth(monthDate))
              ) {
      ...
      const lastMonthRevenue =
        formattedChartData[formattedChartData.length - 1]?.revenus || 0;
      const forecast = [];
      for (let i = 1; i <= 3; i++) {
        const futureMonth = addMonths(new Date(), i);
        const monthName = format(futureMonth, "MMM", { locale: fr });
        const trend =
          (lastMonthRevenue - (formattedChartData[0]?.revenus || 0)) / 6;
        const predictedRevenue = Math.max(
          0,
          lastMonthRevenue + monthlyRecurringRevenue * i + trend * i
        );
        forecast.push({
          name: monthName.charAt(0).toUpperCase() + monthName.slice(1),
          prévision: predictedRevenue,
        });
      }
      ...
      const prevMonthRevenue =
        formattedChartData[formattedChartData.length - 2]?.revenus || 0;
      if (currentMonthRevenue > prevMonthRevenue) {
        insights.push({
          text: "Revenue is up this month. Great job!",
          type: "positive",
        });
      } else {
        insights.push({
          text: "Revenue fell slightly this month. Let's analyze why.",
          type: "negative",
        });
      }
      if (monthlyRecurringRevenue > 0) {
        insights.push({
          text: `Your MRR of ${monthlyRecurringRevenue.toLocaleString(
            "fr-FR"
          )} ${currencySymbol} ensures a solid foundation.`,
          type: "positive",
        });
      }
      if ((statusCounts.overdue || 0) > 0) {
        insights.push({
          text: `You have ${statusCounts.overdue} overdue invoices. Consider resuming them.`,
          type: "warning",
        });
      }
      if (sortedTopClients.length > 0) {
        insights.push({
          text: `Your best customer, ${sortedTopClients[0].name}, is a major asset.`,
          type: "positive",
        });
      }
  ...
  const displayStats = [
    ...
    },
    {
      ...
      value: `${currencySymbol}${stats.clientLTV.toFixed(2)}`,
      change: "",
      icon: Award,
    },
    {
    ...
    <div className="space-y-8">
      ...
      >
        ...
        <p className="text-muted-foreground">
          Your business co-pilot for informed decisions.
        </p>
      </motion.div>
      ...
      >
        <h2 className="text-xl font-bold text-foreground mb-4 flex items-center gap-2">
          <BrainCircuit className="w-6 h-6 text-primary" /> AI synthesis
        </h2>
        {loading ? (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        >
          <h2 className="text-xl font-bold text-foreground mb-6">
            Revenue & New Customers (last 6 months)
          </h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
        ...
        <motion.div
          ...
          className="lg:col-span-3 bg-card/50 backdrop-blur-sm border rounded-xl p-6"
        >
          <h2 className="text-xl font-bold text-foreground mb-6">
            Top 5 Customers by Revenue
          </h2>
          {loading ? (
            <div className="h-72 bg-muted/50 rounded-md animate-pulse"></div>
          ) : (
