...
const BILLING_MANAGEMENT_URL = 'https://billing.stripe.com/p/login/3cIfZi3ML4OwflKciwawo00';
...
const Upgrade = () => {
  ...
  const handleSelectPlan = (plan) => {
    ...
    // For Pro and Business, redirect to billing management
    setIsProcessing(plan.name + billingCycle);
    window.open(BILLING_MANAGEMENT_URL, '_blank');
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-4 bg-background relative">
      ...
      >
        ...
        <div className="flex items-center justify-center gap-4 mt-8">
          <Label htmlFor="billing-cycle-upgrade" className={cn("text-muted-foreground", billingCycle === 'monthly' && 'text-foreground font-semibold')}>{t('monthly')}</Label>
          <Switch id="billing-cycle-upgrade" checked={billingCycle === 'yearly'} onCheckedChange={(checked) => setBillingCycle(checked ? 'yearly' : 'monthly')} />
          <Label htmlFor="billing-cycle-upgrade" className={cn("text-muted-foreground", billingCycle === 'yearly' && 'text-foreground font-semibold')}>{t('yearly')}</Label>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-6xl w-full">
        {plans.map((plan, index) => {
          ...
          const isCurrentPlan = profile?.subscription_plan?.name === plan.name;
          ...
          return (
            <motion.div
              ...
              className={cn("bg-card backdrop-blur-sm border border-border rounded-2xl p-8 flex flex-col shadow-lg relative hover:bg-accent/50 transition-all", plan.name === 'Business' && 'border-primary/50 ring-2 ring-primary/30 bg-accent/30')}
            >
              {plan.name === 'Business' && (
              ...
              <Button
                ...
                disabled={isProcessing === (plan.name + billingCycle) || isCurrentPlan}
              >
                {isProcessing === (plan.name + billingCycle) ? (
