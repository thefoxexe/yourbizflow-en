...
const Dashboard = () => {
  ...
  const getDisplayStats = () => {
    const baseStats = [
      { title: 'Chiffre d\'affaires (Total)', value: `${currencySymbol}${stats.totalRevenue.toLocaleString('fr-FR')}`, change: '', icon: DollarSign, isSensitive: true },
      { title: 'Factures en attente', value: `${currencySymbol}${stats.pendingAmount.toLocaleString('fr-FR')}`, change: `${stats.pendingInvoices} factures`, icon: FileText, isSensitive: true },
      { title: format(currentTime, 'eeee d MMMM', { locale: fr }), value: format(currentTime, 'HH:mm:ss'), change: '', icon: Clock, isTime: true, isSensitive: false }
    ];

    if (isModuleActive('recurring-payments')) {
      baseStats.splice(1, 0, { title: 'Revenus Mensuels Récurrents', value: `${currencySymbol}${stats.monthlyRecurringRevenue.toLocaleString('fr-FR')}`, change: 'MRR', icon: Repeat, isSensitive: true });
    }
  ...
  return (
    <div className="space-y-8">
      <Helmet>
        <title>Tableau de bord - YourBizFlow</title>
        <meta name="description" content="Votre tableau de bord YourBizFlow. Obtenez une vue d'ensemble de votre activité, suivez vos revenus, factures et activités récentes." />
        <meta name="keywords" content="tableau de bord, dashboard, YourBizFlow, revenus, factures, activité" />
      </Helmet>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex justify-between items-center mb-2">
          <div>
            <h1 className="text-3xl font-bold text-foreground">Tableau de bord</h1>
            <p className="text-muted-foreground">Vue d'overview de votre activité business</p>
          </div>
          <TooltipProvider>
            <ShadTooltip>
              ...
              <TooltipContent>
                <p>{isPrivacyMode ? 'Afficher les montants' : 'Masquer les montants'}</p>
              </TooltipContent>
            </ShadTooltip>
          </TooltipProvider>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="lg:col-span-2 bg-card/50 backdrop-blur-sm border rounded-xl p-6">
          <h2 className="text-xl font-bold text-foreground mb-6">Revenus des 6 derniers mois</h2>
                ...
                >
                  ...
                  <div>
                    <p className="text-foreground text-sm font-medium">
                      {activity.type === 'client' ? `Nouveau client: ${activity.name}` : `Facture créée: ${activity.invoice_number}`}
                    </p>
                    <p className="text-muted-foreground text-xs">{formatTimeAgo(activity.date)}</p>
                  </div>
                </motion.div>
              )) : <p className="text-muted-foreground text-sm text-center mt-8">Aucune activité récente.</p>}
            </div>
          )}
        </motion.div>
      </div>
