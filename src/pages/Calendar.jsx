...
const CalendarPage = () => {
  ...
  const fetchEvents = useCallback(async () => {
    ...
    const formattedCalendarEvents = calendarEvents.map((e) => ({
      ...
      borderColor:
        eventCategories[e.category]?.color || eventCategories.other.color,
      extendedProps: {
        type: "event",
  ...
  const handleDateSelect = (selectInfo) => {
    ...
    const isAllDay =
      selectInfo.allDay ||
      (!selectInfo.startStr.includes("T") && !selectInfo.endStr.includes("T"));

    let localStart, localEnd;

    if (isAllDay) {
      localStart = format(selectInfo.start, "yyyy-MM-dd");
      localEnd = selectInfo.end
        ? format(new Date(selectInfo.end.getTime() - 1), "yyyy-MM-dd")
        : localStart;
    } else {
  ...
  const handleEventClick = (clickInfo) => {
    ...
    if (extendedProps.type === "invoice") {
      description = `Invoice for ${
        extendedProps.client || "N/A"
      } in the amount of ${extendedProps.amount}€.`;
    } else {
  ...
  const handleAddEvent = async () => {
    ...
    const startValue = newEvent.allDay
      ? newEvent.start
    ...
    if (checked) {
      setNewEvent({
        ...
        start: newEvent.start
          ? format(new Date(newEvent.start), "yyyy-MM-dd")
          : "",
      ...
      setNewEvent({
        ...
        allDay: false,
        start: newEvent.start
          ? format(new Date(newEvent.start), "yyyy-MM-dd'T'09:00")
          : "",
        end: newEvent.end
          ? format(new Date(newEvent.end), "yyyy-MM-dd'T'10:00")
          : "",
      });
    }
  };
      ...
      >
        <div>
          ...
          <p className="text-muted-foreground">
            Organisez votre temps, ne manquez aucune échéance.
          </p>
        </div>
        <Button
          onClick={() =>
